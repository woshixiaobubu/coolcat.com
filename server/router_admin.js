...
//作品管理：复制作品为默认作品
router.post('/works/setDefaultWork',function(req,res){
...
/*生成24位随机流水号：前8位为时间 中间为平台标志*/
const random_24ID_With_Time_Tag = function() {
    var $chars = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789';
    var maxPos = $chars.length;
    var randomID = parseInt(new Date().getTime()/1000).toString(16).toUpperCase()+"_PuAi_";
    for (i = 0; i < 10; i++) {
...
router.post('/material/backdrop/add', function(req, res) {
    var newFileName = random_24ID_With_Time_Tag();
...
router.post('/material/sound/add', function(req, res) {
    // 仅仅支持wav格式的声音素材
    var newFileName = random_24ID_With_Time_Tag()+".wav";
