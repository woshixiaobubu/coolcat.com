...
table.on('toolbar(TableOne)', function (obj) {
    if (obj.event === 'showMates') {
        ...
        if (_tagId == 0){ layer.msg("请先选择一个标签"); return; }
...
table.on('tool(TableOne)', function (obj) {
    if(obj.event === 'modSoundMate'){
        ...
        if (obj.data.format !== ''){ 
            layer.msg("此格式暂不支持试听，请直接在Scratch中体验！");
            return;
        }
...
table.on('edit(TableOne)', function(obj){
    ...
    if (value==''){ layer.msg('不能为空'); return; }
    ...
    if (field == 'name') {
        ...
        if (!numberTest(value)){ layer.msg("请输入纯数"); return; }

        let v = parseInt(value);

        if (v<1 || 10000000<v){
            layer.msg("此参数建议值：1~1000,0000 之间");
            return;
        }
